...
import { LoginRequest, LoginResponse, RegisterRequest, UserProfile } from '../api/api.types';
...
export class SessionStore {
    ...
    async login(request: LoginRequest): Promise<boolean> {
        ...
        try {
            const response = await firstValueFrom(this.api.post<LoginResponse>('/api/v1/auth/login', request));
    ...
    async register(request: RegisterRequest): Promise<boolean> {
        ...
        this._error.set(null);
        try {
            const profile = await firstValueFrom(this.api.post<UserProfile>('/api/v1/auth/register', request));
            this._user.set(profile);
