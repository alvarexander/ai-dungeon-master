...
describe('ChatStore', () => {
  ...
  it('shows both the player message and the reply after a successful turn', async () => {
    api.post.mockReturnValue(
      of({
        ...
        turn: 1,
        usage: { tokens_in: 10, tokens_out: 20, model_id: 'gemini-3.5-flash', latency_ms: 900 },
        scrubbed: false,
      }),
    );
  ...
  it('hands the text back when the send fails, rather than losing it', async () => {
    api.post.mockReturnValue(
      throwError(() => ({ failure: { message: 'Too many requests.', correlationId: 'abc-123' } })),
    );
  ...
  it('keeps the correlation identifier so the user can quote it', async () => {
    api.post.mockReturnValue(
      throwError(() => ({ failure: { message: 'It broke.', correlationId: 'abc-123' } })),
    );
