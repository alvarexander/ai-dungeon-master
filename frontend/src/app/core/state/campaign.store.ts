...
export class CampaignStore {
    ...
    async load(): Promise<void> {
        ...
        try {
            const page = await firstValueFrom(this.api.get<Page<CampaignSummary>>('/api/v1/campaigns', { limit: 100 }));
    ...
    async create(request: CampaignCreateRequest): Promise<CampaignDetail | null> {
        ...
        this._error.set(null);
        try {
            const created = await firstValueFrom(this.api.post<CampaignDetail>('/api/v1/campaigns', request));
            await this.load();
            return created;
        } catch (error) {
            this._error.set(messageFrom(error));
    ...
    async get(campaignId: string): Promise<CampaignDetail | null> {
        try {
            return await firstValueFrom(this.api.get<CampaignDetail>(`/api/v1/campaigns/${campaignId}`));
        } catch (error) {
            this._error.set(messageFrom(error));
            return null;
        }
     ...
     */
    async createCharacter(campaignId: string, request: CharacterCreateRequest): Promise<CharacterDetail | null> {
        this._loading.set(true);
        this._error.set(null);
        try {
    ...
    async updateCharacter(
        ...
        try {
            ...
            this._characters.update((list) =>
                list.map((character) => (character.character_id === characterId ? updated : character))
            );
            return updated;
        } catch (error) {
            this._error.set(messageFrom(error));
