...
const TONES: ReadonlyArray<{ value: Tone; label: string; description: string }> = [
  { value: 'heroic', label: 'Heroic', description: 'Courage is rewarded. The world is worth saving.' },
  { value: 'gritty', label: 'Gritty', description: 'Resources matter, wounds linger, few people are purely good.' },
  { value: 'comedic', label: 'Comedic', description: 'Absurd but consistent. Jokes come from the situation.' },
  { value: 'horror', label: 'Horror', description: 'More is withheld than shown. You feel watched before you see anything.' },
  { value: 'mystery', label: 'Mystery', description: 'Concrete clues to chase. Every scene offers something to notice.' },
];
...
@Component({
  ...
  template: `
    <div class="page page--narrow">
      <header class="page__header">
        <h1>New campaign</h1>
        <p class="page__lead">
          A campaign is one ongoing story. You can change any of this later.
        </p>
      </header>
  ...
  styles: [
    `
      ...
      .tone-set { border: none; padding: 0; margin: 0 0 var(--space-4); }
      .tone {
        display: flex;
        gap: var(--space-3);
      ...
      .tone--selected { border-color: var(--accent); background: var(--surface-raised); }
      .tone span span { display: block; }
      .tone input { margin-top: 4px; }
    `,
  ],
})
