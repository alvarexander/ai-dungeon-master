...
@Component({
    ...
    <div class="page page--narrow">
      @if (character(); as sheet) {
        <header class="page__header">
          ...
          <p class="page__lead">
            Level {{ sheet.level }}
            @if (sheet.ancestry) { {{ sheet.ancestry }} }
        ...
        <section class="card">
          ...
          <p class="small muted" style="margin-bottom: 0">
            The number underneath is the modifier — what gets added to a dice roll. 10 is
            average and gives nothing; every two points above or below shifts it by one.
          </p>
        </section>
        ...
        <p><a [routerLink]="['/campaigns', sheet.campaign_id, 'characters']">Back to characters</a></p>
      } @else if (loading()) {
        <p class="muted">Loading…</p>
      } @else {
        <app-banner kind="danger" title="Not found">
          <p style="margin: 0">
            That character could not be found. It may have been deleted, or it may belong to
            a different account.
          </p>
        </app-banner>
      }
  ...
  styles: [
    `
      ...
      .page--narrow section { margin-bottom: var(--space-4); }
      .sheet__name { font-family: var(--font-narrative); font-size: 2rem; }
      .hp-input { width: 100px; }
      .hp-bar {
      ...
      .hp-bar span { display: block; height: 100%; background: var(--success); transition: width var(--transition); }
      .ability-grid { display: grid; gap: var(--space-3); grid-template-columns: repeat(auto-fit, minmax(84px, 1fr)); margin-bottom: var(--space-4); }
      .ability-box {
        display: flex; flex-direction: column; align-items: center;
        padding: var(--space-3); border: 1px solid var(--border); border-radius: var(--radius);
        background: var(--surface-sunken);
      }
      .ability-box__label { font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.08em; color: var(--ink-faint); }
      .ability-box__score { font-size: 1.5rem; font-weight: 600; }
      .ability-box__mod { font-size: 0.85rem; color: var(--accent-strong); }
      .narrative { font-family: var(--font-narrative); line-height: 1.75; }
    `,
  ],
})
  ...
  private async load(characterId: string): Promise<void> {
    ...
    try {
      const sheet = await firstValueFrom(
        this.api.get<CharacterDetail>(`/api/v1/characters/${characterId}`),
      );
