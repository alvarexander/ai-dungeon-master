...
@Component({
    ...
    <div class="page">
      ...
      @if (loading()) {
        ...
        <div class="card">
          ...
          <p class="muted">
            You can play without one — the Dungeon Master will help you work out who you are
            as you go. Or make one now if you already know.
          </p>
          <a [routerLink]="['/campaigns', campaignId(), 'characters', 'new']" class="btn btn--primary">
            Create a character
          </a>
          ...
          @for (character of characters(); track character.character_id) {
            <a class="card character" [routerLink]="['/characters', character.character_id]">
              ...
              <p class="small muted">
                ...
                @if (character.ancestry) { {{ character.ancestry }} }
  ...
  styles: [
    `
      .character { text-decoration: none; color: inherit; display: block; }
      .character:hover { border-color: var(--accent); }
      .character__name { font-family: var(--font-narrative); margin-bottom: var(--space-1); }
    `,
  ],
})
