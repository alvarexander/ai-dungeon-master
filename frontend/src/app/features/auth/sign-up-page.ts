      ...
      <form class="card auth__card" (ngSubmit)="submit()">
        <label class="field">
          ...
          <span class="field__hint">
            Letters, numbers, hyphen and underscore. This is stored as ordinary readable
            text, so choose something you are happy to be visible.
          </span>
        </label>
        ...
        <label class="field">
          ...
          @if (passwordTooShort()) {
            <span class="field__error">
              At least {{ minLength }} characters. Length matters far more than
              punctuation — a phrase you can remember beats a short jumble you cannot.
            </span>
          } @else {
            <span class="field__hint">
              At least {{ minLength }} characters. Hashed with Argon2id, which is
              deliberately slow and memory-hungry so that a stolen database cannot be
              cracked at speed.
            </span>
          }
        </label>
